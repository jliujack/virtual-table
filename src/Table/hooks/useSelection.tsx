...
import { Checkbox, CheckboxProps, Dropdown, Menu, Radio } from 'antd';
...
export type ShowSelectionColumnProps<T> = Pick<
  ColumnType<T>,
  'frozen' | 'align' | 'ignoreDisable' | 'ignoreRef' | 'ignoreStopBid'
>;
...
export default function useSelection<RecordType>(
  ...
  const {
    ...
    onSelectNone,
    onSelectMultiple,
    columnWidth: selectionColWidth = 32,
    type: selectionType,
    selections,
    fixed,
  ...
  const checkboxPropsMap = useMemo(() => {
    ...
    flattedData.forEach((record, index) => {
      ...
      const checkboxProps = (getCheckboxProps ? getCheckboxProps({ rowData: record }) : null) || {};
  ...
  const transformColumns = useCallback(
    (columns: ColumnsType<RecordType>): ColumnsType<RecordType> => {
      ...
      // Body Cell
      let renderCell: (cellProps: CellRenderProps<RecordType>) => {
      ...
      // todo: 单元格渲染
      const renderSelectionCell = (cellProps: CellRenderProps<RecordType>) => {
        const { node, checked } = renderCell(cellProps);
        const { rowData, rowIndex } = cellProps;

        if (customizeRenderCell) {
          return customizeRenderCell(checked, rowData, rowIndex!, node);
        }
      ...
      const selectionColumn: ColumnType<RecordType> = {
        ...
        className: `${prefixCls}-selection-column`, // 注意其他地方游泳这个className判断是否为Select列
      ...
      if (expandType === 'row' && columns.length && !expandIconColumnIndex) {
        ...
        return [
          expandColumn,
          { ...selectionColumn, fixed: selectionFixed, ...showSelectionColumnProps },
          ...restColumns,
        ];
      }
      return [

...
const loadQueue = new Set<State>();
let loadCount = 0;

export function useElementSize(onSize: SizeCallback | Falsy): RefCallback<HTMLElement> {
  ...
  function updateSensor() {
    ...
    if (!sensor !== !(elem && state.onSize)) {
      ...
      if (sensor) elem!.appendChild(sensor);
    }
  }
}
...
function flushSizeUpdates() {
  const current = updateQueue;
  updateQueue = [];
  current.forEach((state) => {
    const { elem, size, onSize } = state;
    if (onSize)
      onSize(
        // eslint-disable-next-line no-param-reassign
        (state.size = elem ? { width: elem.clientWidth, height: elem.clientHeight } : null),
        size,
        elem,
      );
  });
